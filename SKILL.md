...
6. Start the site with `node <project>/web/server.mjs`, keep it alive, and open the URL it prints. Read [references/verification.md](references/verification.md) and then actually look at the thing: render, drag, flip, every slider (including 特效景深 for the effects layer), the finish options, the screenshot button, a ~390 px viewport and the reduced-motion setting. A `ready` flag proves none of that.
